...
from toollib.logger_jd_xy import Logger
...
def get_comments(product_dic, commentID_list, pid, ptitle, item_catagory, page_num, mongo_collection, proxiesIP,
    ...
    try:
        ...
        for comment in comments:
            ...
            commentDict['shop_type'] = int(product_dic['shop_type'])
...
def scraw_web_json(product_dic, commentID_list, pid, ptitle, item_catagory, file_name, mongo_collection, proxiesIP, stnum,
...
def main(log_collection_name, product_index, page_nummber, catalog_collection_name, target_collexction_name,
    ...
    catalog_df = getDataDataFrameFromMysql(isLocal, catalog_collection_name)
    catalog_df['shop_id'] = catalog_df['shop_id'].astype('str')
    catalog_df['original_id'] = catalog_df['original_id'].astype('str')
    catalog_df['name'] = catalog_df['name'].astype('str')
    catalog_df['category'] = catalog_df['category'].astype('str')
    catalog_df['brand_id'] = catalog_df['brand_id'].astype('str')
    catalog_df['shop_type'] = catalog_df['shop_type'].astype('int')
    catalog_df['category_id'] = catalog_df['category_id'].astype('str')

    sellID_list = catalog_df['shop_id'].values
    brand_id_list = catalog_df['brand_id'].values
    shop_type_list = catalog_df['shop_type'].values
    item_id_list = catalog_df['original_id'].values
    item_title_list = catalog_df['name'].values
    catalog_list = catalog_df['category'].values
    category_id_list = catalog_df['category_id'].values
    ...
    if product_index <= bottom_index:
        ...
        target_dict['original_id'] = str(item_id_list[product_index])
        target_dict['name'] = str(item_title_list[product_index])
        target_dict['category'] = str(catalog_list[product_index])
        ...
        scraw_web_json(target_dict, commentID_list, item_id, item_title, item_catagory, product_index, collection, '', startNum,
                       maxPage_num_control,
    ...
    for i in range(product_index + 1, len(catalog_df)):
        ...
        target_dict['shop_type'] = shop_type_list[product_index]
        target_dict['category_id'] = str(category_id_list[product_index])
        scraw_web_json(target_dict, commentID_list, item_id, item_title, item_catagory, i, collection, proxiesIP, startNum,
                       maxPage_num_control, log_collection, indexCollection)
